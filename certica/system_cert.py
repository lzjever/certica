...
class SystemCertManager:
    ...
    def _get_sudo_password(self, prompt: str = "请输入sudo密码: ") -> Optional[str]:
    ...
    def _run_sudo_command(self, command: List[str], password: Optional[str] = None) -> Tuple[bool, str]:
        """
        ...
        if password:
            ...
            try:
                ...
                if process.returncode == 0:
                    ...
                    error_msg = stderr.strip() if stderr else f"Command failed with return code {process.returncode}"
    ...
    def _verify_installation(self, source_cert_path: str, installed_cert_path: str, ca_name: str) -> bool:
        """
        ...
        if not Path(installed_cert_path).exists():
            print(f"  ✗ 验证失败: 证书文件不存在于 {installed_cert_path}")
            return False
        ...
        if not source_fp or not installed_fp:
            print(f"  ⚠️  无法获取证书指纹，跳过指纹验证")
            # Continue with other checks
        elif source_fp != installed_fp:
            print(f"  ✗ 验证失败: 证书指纹不匹配")
            print(f"    源证书: {source_fp[:16]}...")
            print(f"    已安装: {installed_fp[:16]}...")
            return False
        else:
            print(f"  ✓ 证书指纹匹配: {source_fp[:16]}...")
        ...
        try:
            ...
            print(f"  ✓ 证书可被系统读取: {result.stdout.strip()}")
        except Exception as e:
            print(f"  ⚠️  无法验证证书可读性: {e}")
    ...
    def _verify_removal(self, ca_name: str) -> bool:
        ...
        if found_paths:
            print(f"  ✗ 验证失败: 证书文件仍存在于以下位置:")
            for p in found_paths:
                print(f"    - {p}")
            return False
        
        print(f"  ✓ 验证成功: 证书文件已从所有位置删除")
        return True
    
    def install_ca_cert(self, ca_cert_path: str, ca_name: str, password: Optional[str] = None) -> bool:
        """
    ...
    def _install_linux(self, ca_cert_path: str, ca_name: str, password: Optional[str] = None) -> bool:
        ...
        for method in install_methods:
            ...
            try:
                ...
                if not cert_dir.exists():
                    success, error = self._run_sudo_command(['mkdir', '-p', str(cert_dir)], password)
                    if not success:
                        continue
                ...
                # Copy certificate
                success, error = self._run_sudo_command(['cp', ca_cert_path, str(target_path)], password)
                if not success:
                    print(f"Failed to copy certificate ({method_name}): {error}")
                    continue
                ...
                if not success:
                    print(f"Failed to update CA certificates ({method_name}): {error}")
                ...
                if self._verify_installation(ca_cert_path, target_path, ca_name):
                    print(f"✓ CA certificate installed and verified successfully ({method_name})")
                    return True
                else:
                    print(f"⚠️  CA certificate installed but verification failed ({method_name})")
                    # Don't return False here, as the file is installed, just verification failed
                    return True
                
            except Exception as e:
                print(f"Error installing certificate ({method_name}): {e}")
                continue
        
        print("Could not install certificate: No compatible installation method found")
        return False
    ...
    def _remove_linux(self, ca_name: str, password: Optional[str] = None) -> bool:
        ...
        for method in remove_methods:
            ...
            try:
                ...
                print(f"✓ CA certificate removed successfully ({method_name})")
                return True
                
            except Exception as e:
                continue
        
        print("Could not remove certificate: Certificate not found or removal failed")
        return False
    
    def _install_macos(self, ca_cert_path: str, ca_name: str, password: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if success:
                ...
                print(f"Failed to install certificate: {error}")
                return False
        except Exception as e:
            print(f"Error installing certificate: {e}")
            return False
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if success:
                    ...
                    print(f"Failed to remove certificate: {error}")
                    return False
            return False
        except Exception as e:
            print(f"Error removing certificate: {e}")
    ...
    def _install_windows(self, ca_cert_path: str, ca_name: str) -> bool:
        ...
        try:
            ...
            print(f"Failed to install certificate: {error_msg}")
            return False
