...
def cli(ctx, base_dir, skip_check, check_only):
    ...
    if not skip_check:
        if not check_system_requirements():
            click.echo("\nError: System requirements check failed.", err=True)
            click.echo("Use --skip-check to bypass this check (not recommended).", err=True)
            sys.exit(1)
...
def create_ca(ctx, name, org, country, state, city, validity, key_size, template):
    ...
    try:
        ...
        click.echo(f"✓ Root CA created successfully!")
        click.echo(f"  Key: {_format_path(result['ca_key'], base_dir)}")
        click.echo(f"  Cert: {_format_path(result['ca_cert'], base_dir)}")
    except FileExistsError as e:
        click.echo(f"Error: {e}", err=True)
    except Exception as e:
        click.echo(f"Failed to create CA: {e}", err=True)


@cli.command()
@click.option('--ca', required=True, help='CA name to use for signing')
@click.option('--name', required=True, help='Certificate name')
@click.option('--type', type=click.Choice(['server', 'client']), default='server', help='Certificate type')
...
def sign(ctx, ca, name, type, cn, dns, ip, org, country, state, city, validity, key_size, template):
    ...
    if not ca_info:
        click.echo(f"Error: CA '{ca}' not found", err=True)
        return
    ...
    try:
        ...
        base_dir = ctx.obj['base_dir']
        click.echo(f"✓ Certificate signed successfully!")
        click.echo(f"  Key: {_format_path(result['key'], base_dir)}")
        click.echo(f"  Cert: {_format_path(result['cert'], base_dir)}")
    except Exception as e:
        click.echo(f"Failed to sign certificate: {e}", err=True)
...
def list_certs(ctx, ca):
    ...
    if ca:
        ...
        if not certs:
            click.echo(f"No certificates found for CA '{ca}'")
            return
        ...
        for cert in certs:
            ca_name = cert.get('ca_name', 'unknown')
...
def create_template(ctx, name, org, country, state, city, validity, key_size):
    ...
    path = template_manager.create_template(
        name, org, country, state, city, validity, key_size
    )
    click.echo(f"✓ Template created: {_format_path(path, base_dir)}")
...
def install(ctx, ca, password):
    ...
    if password is None:
        password = click.prompt("请输入sudo密码", hide_input=True, default="")
        if not password:
            click.echo("需要sudo权限，但未提供密码", err=True)
            return
    
    if system_cert_manager.install_ca_cert(ca_info['cert'], ca_info['name'], password):
        click.echo(f"✓ CA certificate '{ca}' installed to system")
    else:
        click.echo(f"Failed to install certificate. Please check password or sudo privileges.", err=True)
...
def remove(ctx, ca, password):
    ...
    if system_cert_manager.remove_ca_cert(ca, password):
        click.echo(f"✓ CA certificate '{ca}' removed from system")
    else:
        click.echo(f"Failed to remove certificate. Please check password or certificate not found.", err=True)
